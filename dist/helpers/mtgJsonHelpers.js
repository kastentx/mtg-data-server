...
exports.getCardByUuid = getCardByUuid;
...
const largeDataHelpers_1 = require("./largeDataHelpers");
...
const DATA_PATH = path_1.default.join(DATA_DIR, DATA_FILE);
...
async function checkLocalFileModified() {
    try {
        const stats = await promises_1.default.stat(DATA_PATH);
...
async function downloadCardData() {
    try {
        ...
        const data = await response.arrayBuffer();
        // Create temporary zip file
        const tempZipPath = path_1.default.join(DATA_DIR, `${DATA_FILE}.zip`);
...
async function getCardDatabase() {
    if (!cardDb) {
        if (!(0, fs_1.existsSync)(DATA_PATH)) {
...
async function getCardByUuid(uuid) {
    try {
        const db = await getCardDatabase();
        const card = await db.get('SELECT data FROM cards WHERE uuid = ?', [uuid]);
        if (!card)
            return null;
...
async function getCardsByUuid(uuids) {
    try {
        ...
        try {
            const pricingDb = await (0, largeDataHelpers_1.getPricingDatabase)();
            ...
            if (pricingTableCheck) {
                ...
                const prices = await pricingDb.all(`SELECT * FROM cardPrices WHERE gameAvailability="paper" AND currency="USD" AND date = (SELECT MAX(date) FROM cardPrices) AND uuid IN (${uuids.map(() => '?').join(',')})`, uuids);
